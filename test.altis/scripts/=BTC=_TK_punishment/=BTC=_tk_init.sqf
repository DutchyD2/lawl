...
BTC_fnc_tk_PVEH =
{
	...
	hint format ["%1 has teamkilled %2 and was punished",_name,_punisher];
	...
	if (_name != name _body && BTC_vip find (name _killer) == -1) then
	{
		if (side _killer == BTC_side && _veh_body != _veh_killer) then
		{
			...
			_killer spawn 
			{
				...
				{
				    hint format ["%1 teamkilled you! You can decide to punish him by action menu", name _this];
				    WaitUntil {Alive player};
				    _action = player addAction [("<t color=""#ED2744"">") + ("Punish " + name _this) + "</t>","scripts\=BTC=_TK_punishment\=BTC=_punish_action.sqf",[name _this], 8, true, true, "", "true"];
				    _timeout = time + 30;
